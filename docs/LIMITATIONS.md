...
The results concern a packaged deterministic two-object collision simulator, the tested model family, the released checkpoints, and the declared velocity and position edits, horizons, thresholds, and controls.

## Rank and representation

- Rank 4 is the smallest passing tested intervention rank for the original registered single-component grid. It is not an intrinsic dimension of the environment or a complete-state dimensionality claim.
- The same fitting procedure is applied separately to each checkpoint. This does not imply that checkpoints share one raw hidden-space basis.
- The rank-4 carrier is supported as a compact intervention-entry interface. Full recurrent propagation is not shown to be closed in four dimensions.
...
Position editing remains a negative specificity contrast, not a second successful carrier family. Its failure under the registered controls does not imply that every possible position intervention must fail.
